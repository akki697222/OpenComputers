...
abstract class SimpleBlock(props: Properties) extends ContainerBlock(props) {
  ...
  @OnlyIn(Dist.CLIENT)
  override def appendHoverText(stack: ItemStack, context: Item.TooltipContext, tooltip: util.List[Component], flag: ITooltipFlag): Unit = {
    tooltipHead(stack, context, tooltip, flag)
    tooltipBody(stack, context, tooltip, flag)
    tooltipTail(stack, context, tooltip, flag)
  }

  protected def tooltipHead(stack: ItemStack, context: Item.TooltipContext, tooltip: util.List[Component], flag: ITooltipFlag): Unit = {
  }

  protected def tooltipBody(stack: ItemStack, context: Item.TooltipContext, tooltip: util.List[Component], flag: ITooltipFlag): Unit = {
    for (curr <- Tooltip.get(getClass.getSimpleName.toLowerCase)) {
      tooltip.add(Component.literal(curr).setStyle(Tooltip.DefaultStyle))
    }
  }

  protected def tooltipTail(stack: ItemStack, context: Item.TooltipContext, tooltip: util.List[Component], flag: ITooltipFlag): Unit = {
  }
