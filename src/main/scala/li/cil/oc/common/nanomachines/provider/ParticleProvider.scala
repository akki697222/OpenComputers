  ...
  override def writeBehaviorToNBT(behavior: Behavior, nbt: CompoundTag): Unit = {
    behavior match {
      case particles: ParticleBehavior =>
        nbt.putString("effectName", BuiltInRegistries.PARTICLE_TYPE.getKey(particles.effectType).toString)
      case _ => // Wat.
    }
  }

  override def readBehaviorFromNBT(player: Player, nbt: CompoundTag): Behavior = {
    val effectType = BuiltInRegistries.PARTICLE_TYPE.get(ResourceLocation.tryParse(nbt.getString("effectName")))
