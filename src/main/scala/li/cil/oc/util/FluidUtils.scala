...
object FluidUtils {
  ...
  def lookupFluidForBlock(block: Block): Fluid = block match {
    case fluid: LiquidBlock => fluid.defaultBlockState.getFluidState.getType
    case _ => null
  }
  ...
  private class LiquidBlockWrapper(val position: BlockPosition, val block: LiquidBlock) extends BlockWrapperBase {
    ...
    override def isFluidValid(tank: Int, fluid: FluidStack): Boolean = lookupFluidForBlock(block).isSame(fluid.getFluid)
