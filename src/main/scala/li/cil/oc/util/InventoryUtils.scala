      ...
      Option(world.getCapability(Capabilities.ItemHandler.BLOCK, pos, side)) match {
        ...
        case _ =>
          world.getEntitiesOfClass(classOf[Entity], position.bounds)
            .filter(e => e.isAlive && e.getCapability(Capabilities.ItemHandler.ENTITY, null) != null)
            .map(a => EntityInventorySource(a, side, a.getCapability(Capabilities.ItemHandler.ENTITY, null)))
